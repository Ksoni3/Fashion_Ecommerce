...
const Card = ({ singleData }) => {
  ...
  return (
    <div>
      <div className="col-span-1 h-32rem w-[90%] mx-auto text-center flex flex-col items-center ">
        <div className="bg-white h-[200px] w-[90%] rounded-lg flex justify-center items-center">
          <Link to={`/products/${id}`}>
            <img
              src={image}
              alt={title}
              className="h-[150px]  w-[70%] mx-auto"
            />
          </Link>
        </div>
        <div>
          ...
          <button
            ...
            className="bg-blue-500 text-white h-[40px] w-[100px] rounded-md"
          >
            Add To Cart
          </button>
        </div>
